...
export async function createApp(config: AppConfig): Promise<Application> {
  ...
  function validateBuildingRecord(
    record: Partial<Omit<Building, "rooms">>,
  ): boolean {
    return (
  ...
  function validateRoomRecord(
    parentBuilding: string,
    record: Partial<Room> & { building?: string },
  ): boolean {
    return (
  ...
  async function processCourseOfferingsDataset(
    job: DatasetJob,
    zip: JSZip,
  ): Promise<void> {
    const courseFiles = Object.values(zip.files).filter(
      (file) => !file.dir && file.name.startsWith("courses/"),
    );
    ...
    for (const file of courseFiles) {
      ...
      for (const rec of parsed.result) {
        ...
        if (!course) {
          ...
          const currentTitleYear = ((course as any).titleYear ??
            -Infinity) as number;
          ...
          if (
            changed &&
            !coursesAddedThisJob.has(courseId) &&
            !coursesModifiedThisJob.has(courseId)
          ) {
  ...
  async function processFacilitiesDataset(
    job: DatasetJob,
    zip: JSZip,
  ): Promise<void> {
    ...
    const buildingTable = findFirst(
      indexDoc,
      (node) => node?.tagName === "table" && hasClass(node, "views-table"),
    );
    if (!buildingTable) {
    ...
    for (const row of findAll(
      ...
      for (const roomRow of findAll(
        roomTable,
        (node) => node?.tagName === "tr",
      )) {
        const roomCells = directChildCellsByClass(roomRow);
        const numberLink = findFirst(
          roomCells["views-field-field-room-number"],
          (node) => node?.tagName === "a",
        );
        const hrefLink = findFirst(
          roomCells["views-field-nothing"],
          (node) => node?.tagName === "a",
        );
        const seatsText = textContent(
          roomCells["views-field-field-room-capacity"],
        ).trim();
        const furniture = textContent(
          roomCells["views-field-field-room-furniture"],
        ).trim();
        const type = textContent(
          roomCells["views-field-field-room-type"],
        ).trim();
        ...
        if (
          !numberLink ||
          !hrefLink ||
          !validateRoomRecord(shortname, candidate)
        ) {
          continue;
        }
  ...
  function sectionToResponseGETandPUT(
    courseId: string,
    sectionId: string,
    section: Section,
  ) {
  ...
  function roomToResponseGETandPUT(
    buildingId: string,
    roomId: string,
    room: Room,
  ) {
  ...
  function parsePagination(_req: any): PaginationResult {
    ...
    if (Object.keys(params).length > 0) {
      return {
        ok: false,
        body: { error: "Invalid request parameters", params },
      };
    }
  ...
  function sectionToResponse422(_req: any) {
    ...
    if (
      !errors.pass &&
      (!Number.isFinite(pass) || !Number.isInteger(pass) || pass < 0)
    ) {
    ...
    if (
      !errors.fail &&
      (!Number.isFinite(fail) || !Number.isInteger(fail) || fail < 0)
    ) {
    ...
    if (
      !errors.audit &&
      (!Number.isFinite(audit) || !Number.isInteger(audit) || audit < 0)
    ) {
  ...
  async function handleDatasetUpload(
    ...
    if (kind === undefined || kind === null) {
      ...
      typeof kind !== "string" ||
      !options.allowedKinds.includes(kind as DatasetKind)
    ) {
  ...
  app.post("/api/v1/search", async (_req, res) => {
    ...
    if ("ORDER" in options) {
      if (
        ...
        !columns.includes(options.ORDER)
      ) {
  ...
  app.post("/api/v2/search", async (_req, res) => {
    ...
    const isApplyKeySyntax = (value: unknown): value is string => {
      return (
        typeof value === "string" && value.length > 0 && !value.includes("_")
      );
    };
    ...
    const validateFilter = (filter: any): string | null => {
      ...
      if (op === "AND" || op === "OR") {
        ...
        for (const child of val) {
          ...
          if (childErr !== null) {
            if (
              childErr === "WHERE must be an object with at most one FILTER"
            ) {
              return `${op} must be a non-empty array of FILTER objects`;
            }
            return childErr;
    ...
    const where = query.WHERE;
    const options = query.OPTIONS;
    const hasTransformations = Object.prototype.hasOwnProperty.call(
      query,
      "TRANSFORMATIONS",
    );
    ...
    if ("ORDER" in options) {
      ...
      if (typeof order === "string") {
        ...
        if (
          !Array.isArray(keys) ||
          keys.length === 0 ||
          keys.some((key: any) => typeof key !== "string")
        ) {
    ...
    if (
      referencedKinds.size > 1 ||
      (referencedKinds.size === 1 && !referencedKinds.has(kind as SearchKind))
    ) {
      invalidQuery(
        "Cannot mix course_offerings and facilities fields in one query",
      );
    ...
    if (kind === "course_offerings") {
      for (const course of Object.values(
        database.courses as Record<string, any>,
      )) {
        for (const section of Object.values(
          course.sections as Record<string, any>,
        )) {
          rows.push({
            title: course.title,
            dept: course.dept,
      ...
      for (const [buildingId, building] of Object.entries(
        database.buildings as Record<string, any>,
      )) {
        for (const room of Object.values(
          building.rooms as Record<string, any>,
        )) {
          rows.push({
            address: building.address,
            building: buildingId,
    ...
    if ("ORDER" in options) {
      const order = options.ORDER;
      const orderKeys =
        typeof order === "string" ? [order] : ((order as any).keys as string[]);
      const direction =
        typeof order === "string"
          ? "UP"
          : ((order as any).dir as "UP" | "DOWN");
  ...
  app.get("/api/v1/courses", async (_req, res) => {
    ...
    const allItems = courseIds.map((courseId) =>
      courseToResponseGETandPUT(courseId, database.courses[courseId]),
    );
  ...
  app.get("/api/v2/buildings/:building/rooms", async (_req, res) => {
    ...
    const allItems = roomIds.map((roomId) =>
      roomToResponseGETandPUT(buildingId, roomId, building.rooms[roomId]),
    );
    const items = allItems.slice(offset, offset + limit);
  ...
  app.get("/api/v2/buildings/:building/rooms/:room", async (_req, res) => {
    ...
    if (!room) {
      res
        .status(404)
        .json({ error: "Not found", message: `no room with id '${roomId}'` });
      return;
    }
  ...
  app.put("/api/v1/courses/:course", async (_req, res) => {
    ...
    if (course) {
      ...
      const courseObject = courseToResponseGETandPUT(
        courseId,
        database.courses[courseId],
      );
  ...
  app.put("/api/v1/courses/:course/sections/:section", async (_req, res) => {
    ...
    if (section) {
      ...
      const sectionObject = sectionToResponseGETandPUT(
        courseId,
        sectionId,
        course.sections[sectionId],
      );
  ...
  app.get("/api/v1/courses/:course/sections", async (_req, res) => {
    ...
    const sectionList = Object.entries(course.sections)
      .map(([sectionId, section]) =>
        sectionToResponseGETandPUT(courseId, sectionId, section),
      )
